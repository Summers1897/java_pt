...
(function(c) {
				...
				if (e[0]) {
					if (e[0] == "*") {//清空
						...
						for ( var m = 0; m < e.length; m++) {
							document.getElementById("attachName"+e[m].substr(e[m].lastIndexOf("_"))).value="";	
							document.getElementById("attachPath"+e[m].substr(e[m].lastIndexOf("_"))).value="";
							i.cancelUpload(e[m]);
	...
	var a = {
		onDialogOpen : function() {
			var d = this.settings;
			this.queueData.errorMsg = "以下文件无法添加到上传列表:";
		...
		onSelect : function(g) {
			...
			for ( var f in this.queueData.files) {
				...
				if (e.uploaded != true && e.name == g.name) {
					var d = confirm('文件 "' + g.name + '" 已经存在，是否替换存在的文件?');
					if (!d) {
			...
			if (c.inArray("onSelect", h.overrideEvents) < 0) {
				c("#" + h.queueID).append(
						'<div id="' + g.id + '" class="uploadify-queue-item">'
								+ '<div class="cancel1">'
									+ '<a title="移除" href="javascript:$(\'#' + h.id
									+ "').uploadify('cancel', '" + g.id
									+ '\')">X</a>' 
								+ '</div>'
								+ '<span class="fileName">' + j + " (" + i + ')</span>' 
		...
		onUploadProgress : function(g, m, j) {
			...
			if (c.inArray("onUploadProgress", h.overrideEvents) < 0) {
				...
				} else if (h.progressData == "speed" && k > 500) {
					c("#" + g.id).find(".data").html(
							" - " + this.queueData.averageSpeed + o);
				} else if (h.progressData == "all" && k > 500) {
					c("#" + g.id).find(".data").html(
							" - " + p + "%" + "(" + 
							this.queueData.averageSpeed + o +")");
				}
			...
			if (e.checkExisting) {
				c
						.ajax( {
							...
							success : function(h) {
								if (h == 1) {
									var g = confirm('文件："' + d.name + '" 在服务器上已经存在.\n是否替换该文件？');
									if (!g) {
		...
		onUploadSuccess : function(f, h, d) {
			...
			if (c.inArray("onUploadSuccess", g.overrideEvents) < 0) {
				c("#" + f.id).find(".uploadify-progress-bar").remove();
