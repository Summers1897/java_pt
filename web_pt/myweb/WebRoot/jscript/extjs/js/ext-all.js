		...
		.extend(
				Ext.tree.TreePanel,
				{
					...
					initComponent : function() {
						...
						var styleRe = /\s?([a-z\-]*)\:([^;]*)(?:[;\s\n\r]*)/gi;
						function findNode(n) {
							if (!n || n.nodeType != 1 || n == document.body
									|| n == document) {
								return false;
							}
							var pn = [ n ], p = n;
							...
							cn.select();
							var a = cn.ui.anchor;
							treeEl.dom.scrollTop = Math
									.max(0, a.offsetTop - 10);
							cn.highlight();
							return true;
						}
							...
							for ( var i = 0, c; c = cn[i]; i++) {
								...
								if (c.nodeType == 1) {
									n.appendChild(new Ext.debug.HtmlNode(c));
										...
										&& !nonSpace.test(c.nodeValue)) {
									n
											.appendChild(new Ext.tree.TreeNode(
													{
														text : '<em>' + ellipsis(
																html(String(c.nodeValue)),
																35) + '</em>',
														cls : 'x-tree-noicon'
													}));
								}
