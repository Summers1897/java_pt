...
<html>
  <head>
	...
	function SelectOrgan(){
	    ...
	    var tree = new Ext.tree.TreePanel({
	        ...
	        rootVisible:false,
	        autoScroll:true,
	        loader: new Ext.tree.TreeLoader({dataUrl:'<%=basePath%>organ/tree.action'}),
  ...
  <body style="height:100%;">
		...
		<table id="userInfo" width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#ffffff"  class="tab011">
			...
			<td><input type="text"  id="organName" readonly onclick="SelectOrgan()"  class="selectInput" value="${user.sysOrgan.orgCname}">
			<span style="color:red;padding-left:5px">*</span>
			<input type="hidden" id="organId" name="user.sysOrgan.id"  value="${user.sysOrgan.id}"></td></tr>
			<tr class="oddline"><td height="30" align="right" style="width:8%;padding-right:10px" class="black">员工编号：</td>
			<td><input type="text" name="user.usrCode" id="usrCode" class="txt" value="${user.usrCode}" >
			<span style="color:red;padding-left:5px">*</span></td></tr>
			<tr class="oddline"><td height="30" align="right" style="width:10%;padding-right:10px" class="black">登录名称：</td>
			<td><input type="text" name="user.usrLoginname" id="usrLoginname" value="${user.usrLoginname}" class="txt">
			<span style="color:red;padding-left:5px">*</span></td></tr>
			<tr class="oddline"><td height="30" align="right" style="padding-right:10px" class="black">登录密码：</td>
			<td><input type="password" id="usrLoginpwd" name="user.usrLoginpwd" value="${user.usrLoginpwd}" class="txt">
			<span style="color:red;padding-left:5px">*</span>
			</td></tr>
			<tr class="oddline"><td height="30" align="right" style="padding-right:10px" class="black">姓 名：</td>
			<td><input type="text" name="user.usrName" id="usrName" value="${user.usrName}" class="txt">
			...
			<option value="1" <s:if test="user.usrGender==1">selected</s:if>>男</option>
			<option value="0" <s:if test="user.usrGender==0">selected</s:if>>女</option>
			</select>
			</td></tr>
			<tr class="oddline"><td height="30" align="right" style="padding-right:10px" class="black">职 务：</td>
			<td><input type="text" name="user.usrPosition" value="${user.usrPosition}" class="txt"></td></tr>
			<tr class="oddline"><td height="30" align="right" style="padding-right:10px" class="black">电子邮件：</td>
			<td><input type="text" name="user.usrEmail" value="${user.usrEmail}" class="txt"></td></tr>
			...
			<option value="1" <s:if test="user.usrState==1">selected</s:if>>启用</option>
			<option value="0" <s:if test="user.usrState==0">selected</s:if>>禁用</option>
			</select>
			</td></tr>
			<tr class="oddline">
			  <td height="30"  align="right" style="padding-right:10px" class="black">备 注：</td><td>
			     <textarea rows="5" cols="35" name="user.usrRemark" class="txt">${user.usrRemark}</textarea>
			  </td>
			</tr>
		</table> 
		<table id="roleInfo" width="100%" border="0" style="background-color: #FFFFFF;padding:5px 0px 5px 0px;display:none">
			<tr>
			<td width="200px" height="380px">
			<select id="orginalList" multiple="multiple"  ondblclick="moveSelecteds('orginalList','selfRoleList',true)" style="width:200px;height:380px">
			...
			<td height="380px">
			<select id="selfRoleList" name="selfRoleList" multiple="multiple" style="width:200px;height:380px;" ondblclick="moveSelecteds('selfRoleList','orginalList',true)">
			<s:iterator value="selfRoleList">
				<option value="${id}">${roleName}</option>
			</s:iterator>
			</select>
			</td>
